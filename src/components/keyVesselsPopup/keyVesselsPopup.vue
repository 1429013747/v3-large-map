...
const handleDetail = (vessel) => {
  ...
  selectedVesselData.value = {
    ...
    boatFile: {
      ...
      countryOfOrigin: "中国",
      managerCompanyOfOrigin: "美国",
      operateCompanyOfOrigin: "美国",
      DocCompanyOfOrigin: "美国",
      boatSignCompanyOfOrigin: "美国",
      technologyCompanyOfOrigin: "美国",
...
const handleCancelKeyVessels = (vessel) => {
  vessel.isKey = false;
  vessel.category = "suspicious";
  emit("cancel-key", vessel);
  console.log("取消重点:", vessel);
};

// 从详情弹窗设置重点船舶
const handleSetKeyVesselsFromDetail = (vesselsData) => {
  const vessel = vessels.value.find((v) => v.id === vesselsData.id);
  vesselsVisible.value = true;
  selectedVessel.value = vessel;
  if (vessel) {
    emit("set-key", vessel);
    console.log("从详情弹窗设置重点:", vessel);
  }
};

const onVesselsSubmit = () => {
  selectedVessel.value.isKey = true;
  selectedVessel.value.category = "key";
  emit("set-key", selectedVessel.value);
